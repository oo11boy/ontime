...
const SmsReminderSection: React.FC<SmsReminderSectionProps> = ({
  ...
  return (
    <div className="bg-white/5 dark:bg-white/5 rounded-2xl p-5 border border-slate-200 dark:border-white/5">
      <div 
        className="flex items-center justify-between cursor-pointer"
        onClick={() => setSendReminderSms(!sendReminderSms)}
      >
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg transition-colors ${sendReminderSms ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-white/10 text-slate-500 dark:text-gray-400'}`}>
            <Bell className="w-5 h-5" />
          </div>
          <span className={`font-medium transition-colors ${sendReminderSms ? 'text-slate-800 dark:text-white' : 'text-slate-500 dark:text-gray-400'}`}>ارسال پیامک یادآوری</span>
        </div>
        
        <div className={`w-12 h-6 rounded-full transition-all relative ${sendReminderSms ? 'bg-emerald-500' : 'bg-slate-400 dark:bg-gray-600'}`}>
        ...
        <div className="mt-6 space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-gray-400 mb-1">
              <Clock className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400/70" />
          ...
          <div>
            {!reminderMessage ? (
              <button
                ...
                className="w-full bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-dashed border-slate-300 dark:border-white/20 rounded-2xl p-4 flex items-center justify-center gap-3 transition-all group"
              >
                <Plus className="w-5 h-5 text-emerald-600 dark:text-emerald-400 group-hover:scale-110 transition-transform" />
                <span className="text-sm font-medium text-slate-600 dark:text-gray-300">انتخاب متن یادآوری</span>
              </button>
            ) : (
              <div className="relative bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-300 dark:border-emerald-500/20 rounded-2xl p-4">
                <button 
                ...
                <div className="flex items-center gap-2 mb-2 text-[10px] font-bold text-emerald-700 dark:text-emerald-400/70 uppercase tracking-wider">
