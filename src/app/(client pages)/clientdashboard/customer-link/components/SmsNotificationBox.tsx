// src/app/clientdashboard/customer-link/components/SmsNotificationBox.tsx
"use client";

import { useState, useEffect } from "react";
import { Bell, Send, RefreshCw, CreditCard, Loader2, AlertCircle, MessageCircle, ThumbsUp, ChevronDown, ChevronUp } from "lucide-react";
...
export function SmsNotificationBox({ linkId, slug }: SmsNotificationBoxProps) {
  ...
  const [isOpen, setIsOpen] = useState(false); // state برای باز/بسته شدن
  ...
  return (
    <div className="bg-white dark:bg-[#1a1e26] rounded-2xl border border-slate-200 dark:border-white/10 overflow-hidden shadow-sm transition-all">
      {/* هدر باکس - کلیک‌پذیر */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-5 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-white/5 transition-colors text-right"
      >
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-xl bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center">
            <Bell className="w-4 h-4 text-blue-600 dark:text-blue-400" />
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-slate-500 dark:text-gray-400 mt-0.5">
              مدیریت ارسال پیامک‌های اطلاع‌رسانی
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="hidden sm:flex items-center gap-1.5">
            {settings.new_booking_sms_enabled && (
              <div className="w-2 h-2 rounded-full bg-emerald-500" title="پیامک نوبت جدید فعال" />
            )}
            {settings.reschedule_sms_enabled && (
              <div className="w-2 h-2 rounded-full bg-orange-500" title="پیامک تغییر نوبت فعال" />
            )}
            {settings.approval_sms_enabled && (
              <div className="w-2 h-2 rounded-full bg-purple-500" title="پیامک تایید/رد فعال" />
            )}
          </div>
          {isOpen ? (
      ...
      {/* محتوای کشویی */}
      {isOpen && (
        <div className="px-5 pb-5 pt-2 border-t border-slate-100 dark:border-white/10 animate-in slide-in-from-top-2 duration-200">
          {/* باکس اطلاع‌رسانی کسر اعتبار (اجباری) */}
          <div className="bg-amber-50 dark:bg-amber-500/10 rounded-xl p-3 border border-amber-200 dark:border-amber-500/30 mb-4">
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-white/5 rounded-xl mb-2">
            ...
            >
              <span
                className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${
                  settings.new_booking_sms_enabled ? "right-1 " : "left-1"
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-white/5 rounded-xl mb-2">
            ...
            >
              <span
                className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${
                  settings.reschedule_sms_enabled ? "right-1 " : "left-1"
                }`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-white/5 rounded-xl mb-2">
            ...
            >
              <span
                className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${
                  settings.approval_sms_enabled ? "right-1 " : "left-1"
                }`}
              />
            </button>
          </div>
