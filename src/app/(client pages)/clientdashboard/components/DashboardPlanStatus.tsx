// File Path: src/app/(client pages)/clientdashboard/components/DashboardPlanStatus.tsx
...
interface DashboardPlanStatusProps {
  planTitle: string;
  endedAt: string | null | undefined; // تغییر یافته از trialEndsAt
  quotaEndsAt: string | null | undefined;
}
...
export const DashboardPlanStatus: React.FC<DashboardPlanStatusProps> = ({
  ...
  // مرجع اصلی برای محاسبه روزهای باقی‌مانده، تاریخ پایان پلن است
  const remainingDays = calculateRemainingDays(endedAt);
  ...
  if (!endedAt) {
    ...
    timeStatusMessage = `${remainingDays.toLocaleString(
      "fa-IR"
    )} روز تا پایان اعتبار پلن باقیست.`;
          ...
          <div>
            <div className="flex flex-col gap-1 text-right" dir="rtl">
              ...
              <span className="text-lg font-bold text-gray-900">
                {planTitle || "رایگان (تریال)"}
              </span>
            </div>
            <p
              className={`text-[11px] mt-1 text-right ${statusColorClass}`}
              dir="rtl"
            >
              {timeStatusMessage}
            </p>
