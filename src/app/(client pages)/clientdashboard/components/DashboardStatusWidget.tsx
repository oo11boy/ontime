  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      <motion.div
        ...
        className="bg-white/95 dark:bg-[#0F172A]/80 backdrop-blur-3xl border border-slate-200/60 dark:border-white/10 rounded-[2.5rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.08)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all duration-300"
      >
        <div className="flex flex-row items-stretch divide-x divide-x-reverse divide-slate-200/50 dark:divide-white/[0.05]">
          ...
          <div className="flex-1 p-5 sm:p-6 flex flex-col justify-between">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-amber-400/20 to-amber-600/10 dark:from-amber-400/20 dark:to-amber-600/10 flex items-center justify-center border border-amber-500/20">
                <Star
                  size={18}
            ...
            <div className="flex items-center gap-2 mb-6 text-slate-500 dark:text-slate-400 bg-slate-100/80 dark:bg-white/5 w-fit px-3 py-1.5 rounded-full">
              <Clock
                size={12}
                className={
              ...
              <span className="text-[11px] font-bold">
                {remainingDays !== null && remainingDays > 0
                  ? `${formatFa(remainingDays)} روز اعتبار`
                  : "منقضی شده"}
              </span>
            </div>
            ...
            {userType == "user" && (
              <Link href="/clientdashboard/pricingplan">
                <motion.div
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="w-full bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 py-2.5 rounded-2xl flex items-center justify-center gap-2 group transition-all"
                >
                  <Sparkles
                    size={14}
                    className="text-indigo-600 dark:text-indigo-400 group-hover:animate-pulse"
                  />
                  <span className="text-[11px] font-black text-indigo-700 dark:text-indigo-100">
                    ارتقای پلن
                  </span>
                </motion.div>
              </Link>
