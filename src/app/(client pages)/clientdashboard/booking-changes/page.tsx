...
const FixedHeader = ({ pendingCount }: { pendingCount: number }) => {
  return (
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-emerald-100 dark:bg-emerald-500/20 p-2 rounded-xl">
              <AlertTriangle className="text-emerald-600 dark:text-emerald-400 w-5 h-5" />
            </div>
            <div>
              <h1 className="text-md font-bold text-slate-800 dark:text-white">
                مدیریت درخواست‌ها و نوبت‌ها
              </h1>
              <p className="text-xs text-slate-500 dark:text-gray-400 mt-0.5">
                مشاهده و مدیریت تمام درخواست‌های مشتریان
              </p>
            </div>
          </div>
          {pendingCount > 0 && (
  ...
  return (
    <div className="min-h-screen max-w-md m-auto bg-slate-50 dark:bg-[#0f1115] text-slate-800 dark:text-white transition-colors">
      ...
      <InsufficientBalanceModal
        ...
        requiredAmount={2}
      />
<FixedHeader pendingCount={pendingCount} />
          ...
          <div className="mb-4 p-3 bg-yellow-100 dark:bg-yellow-500/20 rounded-xl border border-yellow-200 dark:border-yellow-500/30 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-yellow-700 dark:text-yellow-300 text-sm">
                {smsBalance === 0 
                  ? "اعتبار پیامک شما به پایان رسیده، برای ارسال پیامک تایید یا رد به مشتری اعتبار خود را شارژ کنید"
                  : `اعتبار پیامک شما در حال اتمام است (${smsBalance} واحد باقیمانده)`}
              </span>
            </div>
            <button
