...
export const POST = withAuth(async (req, context) => {
  ...
  try {
    ...
    if (!templateKey) {
      return NextResponse.json(
        { success: false, message: "الگوی پیامک انتخاب نشده است" },
        { status: 400 },
      );
    }
    ...
    if (!template) {
      return NextResponse.json(
        { success: false, message: "الگوی پیامک یافت نشد" },
        { status: 404 },
      );
    }
    ...
    const userRow = userRaw as
      ...
      | undefined;
    const salonName =
      userRow?.business_name?.trim() || userRow?.name?.trim() || "مدیریت";
    const salonAddress = userRow?.business_address?.trim() || "";
    const salonPhone = userRow?.phone?.trim() || "";
    ...
    const queueTasks = recipients.map(async (rec: any, idx: number) => {
      ...
      if (!phone || phone.length !== 10) {
        console.warn(
          `[BULK-SMS] شماره نامعتبر (ردیف ${idx + 1}): ${rec.phone}`,
        );
      ...
      const [logRaw] = await query(
        ...
        [
          ...
          staffId,
          phone,
          `Bulk: ${templateKey} → ${name} (${smsPerMessage} واحد)`,
          smsPerMessage,
          sms_type,
        ],
      );
    ...
    if (successCount === 0 && failedCount > 0) {
      ...
      console.log(
        `💰 Bulk SMS completely failed. Refunded ${deductedInfo.totalNeeded} credits`,
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        message: err.message || "خطای سرور در پردازش ارسال گروهی",
      },
      { status: 500 },
    );
  }
...
async function refundSingleSmsCost(
  logId: number,
  cost: number,
  userId: number,
  staffId: number | null,
...
async function refundBulkSmsCost(
  totalNeeded: number,
  userId: number,
  staffId: number | null,
) {
