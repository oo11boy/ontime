  ...
  try {
    ...
    if (existing && existing.length > 0) {
      return NextResponse.json(
        {
          success: false,
          message: "این آدرس قبلاً توسط شخص دیگری ثبت شده است",
        },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          "خطا در ساخت لینک اختصاصی: " + (error.message || "خطای ناشناخته"),
      },
      { status: 500 },
    );
  }
...
export const PUT = withAuth(async (req: NextRequest, context) => {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      link: { slug: currentSlug, fullUrl: `ontimeapp.ir/c/${currentSlug}` },
    });
  } catch (error: any) {
