...
export function Header({ business, isWorkingNow }: HeaderProps) {
  ...
  // تعیین تصاویر - اولویت با avatar_image سپس logo
  const avatarImage = business.avatar_image || business.logo;
  const hasCover = !!business.cover_image;
  const hasAvatar = !!avatarImage;

  return (
    <div className="relative mb-6">
      {/* بخش کاور - اگر تصویر کاور وجود داشته باشد نمایش داده می‌شود */}
      {hasCover ? (
        <div className="h-72 rounded-b-3xl relative overflow-hidden">
          <img
            src={business.cover_image!}
            alt="Cover"
            className="w-full h-full object-cover"
            onError={(e) => {
              console.error("Cover image failed to load:", business.cover_image);
              e.currentTarget.style.display = "none";
            }}
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-black/20 to-transparent rounded-b-3xl" />
        </div>
      ) : (
      ...
      {/* باکس اطلاعات */}
      <div className="absolute left-0 right-0 px-4 -bottom-16">
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-4 border border-white/20 shadow-2xl">
          <div className="flex items-center gap-3">
            {/* لوگو/آواتار */}
            <div className="w-20 h-20 rounded-xl overflow-hidden border-2 border-white/20 shadow-lg flex-shrink-0 bg-gradient-to-br from-gray-800 to-gray-900">
              {hasAvatar ? (
                <img
                  src={avatarImage!}
                  ...
                  onError={(e) => {
                    ...
                    e.currentTarget.style.display = "none";
                    e.currentTarget.parentElement?.classList.add("bg-gradient-to-br", "from-emerald-500", "to-teal-500");
                  }}
                />
              ) : (
