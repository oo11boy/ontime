...
import { Plus, Bell, Clock, MessageSquare, Trash2, Loader2, HelpCircle, Info, X, ChevronLeft, CalendarDays } from "lucide-react";
...
export default function AdminTemplatesPage() {
  ...
  // فیلدهای فرم (اضافه شدن sub_type برای تفکیک امروز/فردا)
  const [formData, setFormData] = useState({
    name: "",
    type: "reserve", // reserve, reminder, generic
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setShowModal(false);
        setFormData({ name: "", type: "reserve", sub_type: "none", payamresan_id: "", content: "" });
        fetchTemplates();
      } else {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/admin/smstemplates?id=${id}`, { method: "DELETE" });
      if (res.ok) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-2 mr-1">پیکربندی پترن‌های ارسالی بر اساس زمان‌بندی دقیق</p>
          </div>
          <button 
                ...
                <div className="flex justify-between items-start mb-5 relative z-10">
                  <div className="flex flex-col gap-2">
                    <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase w-fit ${
                      tpl.type === 'reserve' ? 'bg-blue-500/10 text-blue-400' : 
                      tpl.type === 'reminder' ? 'bg-amber-500/10 text-amber-400' : 'bg-purple-500/10 text-purple-400'
                    }`}>
                      {tpl.type === 'reserve' ? '🔔 ثبت رزرو' : tpl.type === 'reminder' ? '⏰ یادآوری' : '📩 عمومی'}
                    </span>
                    {tpl.sub_type !== 'none' && (
                  ...
                  <button onClick={() => handleDelete(tpl.id)} className="bg-red-500/10 p-2 rounded-lg text-gray-500 hover:text-red-500">
                    <Trash2 size={18} />
                  </button>
                </div>
                
                <h3 className="font-bold text-xl mb-3 group-hover:text-emerald-400 transition-colors line-clamp-1">{tpl.name}</h3>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 z-50">
          <div className="bg-[#181818] w-full max-w-xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl relative animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[95vh]">
            <button onClick={() => setShowModal(false)} className="absolute left-6 top-6 text-gray-500 hover:text-white transition-all">
              <X size={28} />
            </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="md:col-span-2">
                  <label className="block text-sm text-gray-400 mb-2 mr-1">عنوان الگو (مثلاً: یادآوری فردا)</label>
                  <input 
                    required 
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-2 mr-1">نوع اصلی</label>
                  <select 
                    ...
                    onChange={(e) => {
                        ...
                        setFormData({...formData, type: val, sub_type: val === 'reminder' ? 'tomorrow' : 'none'});
                    }}
                  >
                ...
                {/* فیلد جدید: انتخاب امروز یا فردا (فقط برای یادآوری‌ها) */}
                <div className={formData.type === 'reminder' ? "block" : "invisible"}>
                  <label className="block text-sm text-gray-400 mb-2 mr-1">هدف زمانی (ارسال برای...)</label>
                  <select 
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-2 mr-1">Pattern ID (از پنل IPPanel)</label>
                <input 
                  required 
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  <label className="text-sm text-gray-400 mr-1">متن الگو (جهت اطلاع)</label>
                  <button type="button" onClick={() => setShowHelp(!showHelp)} className="text-[11px] flex items-center gap-1.5 text-blue-400 bg-blue-500/5 px-3 py-1 rounded-full">
                    <HelpCircle size={14} /> راهنما
                  </button>
                </div>
                
                {showHelp && (
                  <div className="bg-blue-500/5 border border-blue-500/20 p-4 rounded-2xl mb-4 text-[10px] grid grid-cols-2 gap-2">
                    <div className="col-span-2 text-blue-300 font-bold mb-1">متغیرهای مجاز:</div>
                    <code className="bg-black/40 p-1.5 rounded border border-white/5">%name% , %date%</code>
                    <code className="bg-black/40 p-1.5 rounded border border-white/5">%time% , %link%</code>
                  </div>
                )}
                ...
                <textarea 
                  ...
                  placeholder="یادآوری نوبت فردا ساعت %time%..." 
                />
              </div>

              <div className="flex gap-4">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 bg-gray-900 text-gray-400 py-4 rounded-2xl font-bold">انصراف</button>
                <button type="submit" className="flex-1 bg-emerald-600 text-white py-4 rounded-2xl font-black shadow-lg shadow-emerald-900/40 flex items-center justify-center gap-2">
