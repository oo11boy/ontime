...
import {
  ...
  Scissors,
  AlertCircle,
  ChevronRight, // تغییر به راست برای زبان فارسی
  X,
  RefreshCw,
  CheckCircle,
  Copy,
...
export default function CustomerBookingPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0f1115] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="mt-6 text-gray-400 font-medium animate-pulse text-lg">در حال دریافت اطلاعات نوبت...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-white selection:bg-emerald-500/30 pb-10" dir="rtl">
      ...
      <div className="relative h-48 bg-gradient-to-b from-emerald-600/20 to-transparent">
        <div className="max-w-md mx-auto px-4 pt-6 flex justify-between items-center">
          ...
          <h1 className="text-lg font-bold opacity-80">جزئیات رزرو نوبت</h1>
          <div className="w-10"></div>
        </div>

        {/* Business Avatar/Icon Card */}
        <div className="absolute -bottom-10 left-0 right-0 px-4">
          <div className="max-w-md mx-auto bg-[#1a1d24] border border-white/10 rounded-3xl p-5 shadow-2xl flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-black text-white">{booking.businessName || "نام کسب‌وکار"}</h2>
              <div className="flex items-center gap-1 text-gray-400 text-sm mt-1">
                <MapPin className="w-3.5 h-3.5" />
                <span>{booking.businessAddress || "آدرس ثبت نشده"}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-md mx-auto px-4 mt-16 space-y-5">
        ...
        <div className="flex flex-col gap-4">
          <div className={`flex items-center justify-center gap-2 py-3 px-4 rounded-2xl border backdrop-blur-sm ${
            booking.status === "active" && !isPast ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" :
            booking.status === "cancelled" ? "bg-red-500/10 border-red-500/20 text-red-400" : "bg-gray-500/10 border-gray-500/20 text-gray-400"
          }`}>
            <div className={`w-2 h-2 rounded-full animate-pulse ${
              booking.status === "active" && !isPast ? "bg-emerald-500" : "bg-current"
            }`} />
            <span className="text-sm font-bold">
              {booking.status === "active" && !isPast && "نوبت فعال و تایید شده"}
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={shareBooking} className="flex items-center justify-center gap-2 h-14 bg-[#1a1d24] border border-white/5 rounded-2xl hover:bg-white/5 transition-all active:scale-95 group">
            ...
            <button onClick={copyToClipboard} className="flex items-center justify-center gap-2 h-14 bg-[#1a1d24] border border-white/5 rounded-2xl hover:bg-white/5 transition-all active:scale-95 group">
        ...
        {/* DateTime Card */}
        <section className="bg-[#1a1d24] rounded-[2.5rem] p-6 border border-white/5 relative overflow-hidden group">
          <div className="absolute top-0 left-0 w-32 h-32 bg-emerald-500/5 blur-3xl -translate-x-1/2 -translate-y-1/2" />
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1 bg-white/[0.03] rounded-3xl p-4 text-center border border-white/5 shadow-inner">
              <p className="text-gray-400 text-xs mb-1">تاریخ</p>
              <p className="text-lg font-black text-emerald-400 leading-tight">{persianDate}</p>
            </div>
            <div className="flex-1 bg-white/[0.03] rounded-3xl p-4 text-center border border-white/5 shadow-inner">
              <p className="text-gray-400 text-xs mb-1">ساعت حضور</p>
              <p className="text-lg font-black text-emerald-400 leading-tight">{booking.time}</p>
            </div>
          </div>
          ...
          <div className="mt-4 py-3 px-5 bg-emerald-500/5 rounded-2xl flex items-center justify-between border border-emerald-500/10">
            ...
            <span className="font-bold text-sm">{booking.duration} دقیقه</span>
          </div>
        </section>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {booking.services.length > 0 ? (
              booking.services.map((service, idx) => (
                <span key={idx} className="px-4 py-2 bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 rounded-xl text-xs font-bold shadow-sm">
                  {service}
                </span>
              ))
            ) : (
              <span className="text-gray-500 text-sm italic">خدمات خاصی ذکر نشده</span>
            )}
          </div>
        ...
        <section className="bg-[#1a1d24] rounded-[2.5rem] p-6 border border-white/5">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold tracking-widest" dir="ltr">{booking.clientPhone}</span>
            </div>
          </div>
        </section>
        ...
        <div className="p-5 bg-amber-500/5 border border-amber-500/20 rounded-[2rem] flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs leading-relaxed">
              این لینک به دلایل امنیتی تا تاریخ <span className="font-bold text-amber-200/80">{new Date(booking.expiresAt).toLocaleDateString("fa-IR")}</span> معتبر است.
            </p>
          </div>
        </div>
            ...
            <div className="max-w-md mx-auto flex flex-col gap-3">
              {booking.canReschedule && (
                <button
                  ...
                  className="w-full h-16 bg-white text-black rounded-2xl font-black text-lg flex items-center justify-center gap-3 shadow-xl shadow-white/5 active:scale-95 transition-transform"
                >
