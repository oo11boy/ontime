        ...
        <div className="bg-slate-900 rounded-[3.5rem] lg:rounded-[5rem] p-10 lg:p-28 text-center text-white relative overflow-hidden shadow-[0_50px_100px_-20px_rgba(15,23,42,0.3)] border border-slate-800">
          ...
          <div className="relative z-10">
            ...
            <div className="inline-flex items-center gap-3 bg-emerald-500/10 text-emerald-400 px-6 py-3 rounded-2xl mb-10 border border-emerald-500/20 shadow-inner">
              ...
              <span className="font-black text-xs lg:text-sm uppercase tracking-widest">هدیه ویژه برای شروع هوشمندانه شما</span>
            </div>
            ...
            <div className="mt-20 flex flex-col items-center gap-6">
              <div className="flex -space-x-4 space-x-reverse items-center">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div key={i} className="w-12 h-12 rounded-full border-4 border-slate-900 bg-slate-800 flex items-center justify-center shadow-2xl relative">
              ...
              <p className="text-slate-400 text-base lg:text-lg font-bold leading-relaxed max-w-lg">
                به جمع <span className="text-white">۱,۵۰۰ مجموعه موفق</span> که با آنتایم هزینه‌های خود را کاهش داده‌اند بپیوندید.
              </p>
            </div>
          </div>
        </div>
...
function BenefitCard({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) {
  return (
    <div className="bg-white/5 p-10 rounded-[3rem] backdrop-blur-md border border-white/5 hover:border-white/20 hover:bg-white/10 transition-all duration-500 group/card transform hover:-translate-y-2">
      ...
      <h4 className="text-xl font-black mb-3 text-white group-hover/card:text-emerald-400 transition-colors">{title}</h4>
      <p className="text-slate-400 text-sm font-bold leading-relaxed opacity-80 group-hover/card:opacity-100">{desc}</p>
    </div>
  )
}