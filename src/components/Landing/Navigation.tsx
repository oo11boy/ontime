...
/>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group z-110">
          <div className="w-12 h-12 bg-linear-to-br  rounded-2xl flex items-center justify-center  group-hover:rotate-12 transition-all duration-500">
            <Image src="/icons/icon-192.png" width={100} height={100} alt='لوگو اپلیکیشن نوبت دهی آنتایم' className="text-white rounded-2xl font-black text-2xl italic"/>
          </div>
          <div className="flex flex-col">
            <span className="text-2xl font-black tracking-tighter text-slate-900 leading-none">آنتایم</span>
            <span className="text-[10px] text-slate-500 font-bold mt-1 uppercase tracking-tighter">OnTime App</span>
          </div>
        </Link>
      ...
      `}>
        <div className="flex flex-col h-full p-8 pt-24 gap-4">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">منوی دسترسی سریع</p>
          {menuItems.map((item) => (
            <Link 
              key={item.href}
              href={item.href}
            ...
            >
              <div className="flex items-center gap-4">
                <span className="text-blue-600 bg-white p-2 rounded-xl shadow-sm">{item.icon}</span>
                {item.label}
              </div>
          ...
          <div className="mt-auto pb-10 flex flex-col gap-4">
            ...
            <p className="text-center text-slate-400 text-xs font-bold">پشتیبانی نوبت‌دهی: ۰۹۹۸۱۳۹۴۸۳۲</p>
          </div>
        </div>
      </div>
    </nav>
